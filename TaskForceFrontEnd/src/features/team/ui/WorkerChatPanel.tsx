...
const ThinkBlock: React.FC<{ thoughts: string[]; isRunning: boolean }> = ({ thoughts, isRunning }) => {
  ...
  return (
    <div className="rounded-xl border border-sky-200 dark:border-neutral-700 bg-sky-50/80 dark:bg-neutral-900">
      <button
        type="button"
        className="w-full flex items-center gap-2 px-3 py-2 text-left cursor-pointer"
        onClick={() => setExpanded(value => !value)}
      >
        <span className="text-xs font-semibold text-sky-700 dark:text-neutral-200">{title}</span>
        {isRunning && <Loader2 size={12} className="animate-spin text-sky-600 dark:text-neutral-300" />}
        <span className="text-[11px] text-sky-600 dark:text-neutral-300 ml-auto">
          {expanded ? t('team.collapse') : t('team.expand')}
        </span>
        <ChevronDown size={14} className={clsx("text-sky-500 dark:text-neutral-400 transition-transform", expanded && "rotate-180")} />
      </button>

      {expanded && (
        <pre className="border-t border-sky-200/80 dark:border-neutral-700 px-3 py-2 text-xs text-sky-900 dark:text-neutral-100 whitespace-pre-wrap break-words max-h-52 overflow-auto">
          {content || '...'}
        </pre>
      )}
    </div>
